...
const roStr = `  ro: {
    ...
    home: {
      ...
      sections: {
        ...
        legal: { title: "Asistență Juridică", desc: "Aprobați", items: ["Contracte", "Vize", "Drept penal", "Dispute", "Notari", "Afaceri"] },
         বাণিজ্যিকal: { title: "Comerciale", desc: "Birouri.", items: ["Mutări", "Clădiri Ofertă", "Siguranță", "Electric", "IT", "AC"] }
      },
    ...
    diagnosis: {
      ...
      tool: {
        ...
        disclaimer: "Factura precisă va putea fi vizualizată direct alături de antreprenor.",
...
const koIdx = content.indexOf('\\n  ko: {');
const plIdx = content.indexOf('\\n  pl: {');

if (koIdx > -1 && plIdx > -1) {
  const newContent = content.substring(0, koIdx) + '\\n' + koStr + ',\\n' + roStr + ',' + content.substring(plIdx);
  fs.writeFileSync(file, newContent, 'utf8');
  console.log('Fixed KO and RO dictionary blocks!');
} else {
  console.log('Could not find ko or pl blocks!');
}
