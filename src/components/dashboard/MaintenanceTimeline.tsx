...
export default function MaintenanceTimeline() {
  ...
  if (events.length === 0) {
    return (
      <div className="bg-slate-950/40 backdrop-blur-md rounded-[2rem] p-12 border border-dashed border-white/10 text-center">
        <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4">
          <Calendar className="text-slate-500" size={32} />
        </div>
        <h3 className="text-xl font-bold text-white">{t?.merchant?.maintenance?.emptyTitle || "No Active Schedule"}</h3>
        <p className="text-slate-400 mt-2">{t?.merchant?.maintenance?.emptyDesc || "Add your assets to initialize AI-driven maintenance telemetry."}</p>
        <Link href="/member/profile/assets" className="inline-block mt-6 px-6 py-2 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-500 transition-all text-sm">
          {t?.merchant?.maintenance?.manageCta || "Manage Assets"}
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#0c0c0c] backdrop-blur-2xl rounded-[2.5rem] p-8 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
      <div className="flex items-center justify-between mb-10">
        <div>
          <h3 className="text-2xl font-black text-white tracking-tight flex items-center gap-2">
            {t?.merchant?.maintenance?.title || "Maintenance Timeline"} <span className="text-blue-400">Timeline</span>
          </h3>
          <p className="text-slate-400 text-sm mt-1">{t?.merchant?.maintenance?.subtitle || "AI-predicted service nodes for your elite assets."}</p>
        </div>
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-full text-blue-400 text-xs font-bold">
          <Sparkles size={12} /> {t?.merchant?.ai?.predicted || "AI Predicted"}
        </div>
      </div>
      ...
      <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:h-full before:w-0.5 before:-translate-x-px before:bg-gradient-to-b before:from-blue-500 before:via-white/10 before:to-transparent">
        {events.map((event) => (
          <div key={event?.id || Math.random()} className="relative flex items-start gap-6 group">
            {/* Icon Node */}
            ...
            <div className="flex-1">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-white text-lg tracking-tight">{event?.title || "Unknown Task"}</span>
                  {event?.status === 'OVERDUE' && (
                    <span className="px-2 py-0.5 bg-red-500/20 text-red-400 text-[10px] font-black rounded-lg flex items-center gap-1 border border-red-500/20">
                      <AlertTriangle size={10} /> {t?.merchant?.maintenance?.overdue || "Overdue"}
                    </span>
                  )}
                  {event?.status === 'DUE_SOON' && (
                    <span className="px-2 py-0.5 bg-orange-500/20 text-orange-400 text-[10px] font-black rounded-lg flex items-center gap-1 border border-orange-500/20">
                      <Clock size={10} /> {t?.merchant?.maintenance?.dueSoon || "Due Soon"}
                    </span>
                  )}
                </div>
                <div className="text-slate-500 text-xs font-medium flex items-center gap-1">
                  <Calendar size={12} /> {event?.dueDate ? formatDate(event.dueDate) : "--"}
                </div>
              </div>
              ...
              <div className="bg-white/5 rounded-2xl p-5 border border-white/5 group-hover:border-blue-500/30 group-hover:bg-blue-500/5 transition-all">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div>
                    <p className="text-sm font-bold text-slate-200">{event?.assetName || "Node"}</p>
                    <p className="text-xs text-slate-400 mt-1">{event?.description || "Processing details..."}</p>
                  </div>
                  <Link 
                    href={`/services/results?category=${event?.category || 'Automotive'}&assetId=${event?.assetId || ''}`}
                  ...
                  >
                    {t?.merchant?.maintenance?.bookCta || "Initialize Service"} <ArrowRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                  </Link>
                </div>
              </div>
            </div>
      ...
      {/* Aura Insight Box */}
      <div className="mt-10 p-6 bg-gradient-to-br from-blue-600/10 to-purple-600/10 rounded-[2rem] border border-white/10 relative overflow-hidden group">
        ...
        <div className="relative z-10 flex items-start gap-4">
          ...
          <div>
            <h4 className="font-bold text-lg text-white leading-tight mb-1">{t?.merchant?.ai?.insightTitle || "Aura Insight"}</h4>
            <p className="text-xs text-slate-400 leading-relaxed">
              {t?.merchant?.ai?.insightDesc || "Your maintenance nodes are calibrated based on UK service intervals and asset telemetry. Proactive synchronization ensures maximum asset longevity."}
            </p>
          </div>
        </div>
      </div>
