          ...
          <div className="animate-fade-up glass-panel" style={{ backgroundColor: 'var(--bg-primary)', padding: '2rem', borderRadius: '16px', width: '100%', maxWidth: '500px', position: 'relative' }}>
            ...
            {isSuccess ? (
               <div style={{ textAlign: 'center', padding: '2rem 0' }}>
                 ...
                 <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>{t.merchant.variations.successTitle}</h2>
                 <p style={{ color: 'var(--text-secondary)' }}>{t.merchant.variations.successDesc}</p>
               </div>
            ) : (
               <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                 <div>
                   <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>{t.merchant.variations.modalTitle}</h2>
                   <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>{t.merchant.variations.modalDesc}</p>
                 </div>

                 {/* Photo Proof */}
                 <div>
                   <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>
                     <Camera size={16} /> {t.merchant.variations.photoLabel}
                   </label>
                   <div style={{ border: '2px dashed #cbd5e1', borderRadius: '8px', padding: '2rem', textAlign: 'center', backgroundColor: '#f8fafc', cursor: 'pointer' }}>
                      ...
                      <div style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: 600 }}>{t.merchant.variations.photoPlaceholder}</div>
                 ...
                 <div>
                   <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>
                     <PoundSterling size={16} /> {t.merchant.variations.amountLabel}
                   </label>
                 ...
                 <div>
                   <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>
                     <FileText size={16} /> {t.merchant.variations.reasonLabel}
                   </label>
                   <textarea required value={desc} onChange={e => setDesc(e.target.value)} placeholder={t.merchant.variations.reasonPlaceholder} rows={4} className="input-field" style={{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text-primary)', resize: 'vertical' }}></textarea>
                 </div>
                 ...
                 <div style={{ backgroundColor: '#fffbeb', padding: '1rem', borderRadius: '8px', border: '1px solid #fde68a', fontSize: '0.8rem', color: '#b45309', display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                   ...
                   <span>{t.merchant.variations.warningNote}</span>
                 </div>
                 ...
                 <button type="submit" disabled={isSubmitting || !amount || !desc} className="btn btn-primary" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', width: '100%' }}>
                   {isSubmitting ? t.merchant.variations.submitting : <><Send size={18} /> {t.merchant.variations.sendBtn}</>}
                 </button>
               </form>
            )}
          </div>
