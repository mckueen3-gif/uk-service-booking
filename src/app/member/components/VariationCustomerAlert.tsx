...
export default function VariationCustomerAlert({ merchantName, serviceName, amount, mockDesc, mockPhoto }: { merchantName: string, serviceName: string, amount: string, mockDesc: string, mockPhoto: string }) {
  ...
  const handleReject = () => {
    if (confirm(t.merchant.variations.rejectConfirm)) {
      setIsProcessing(true);
      setTimeout(() => {
        setIsProcessing(false);
        setStatus("REJECTED");
  ...
  if (status === "APPROVED") {
    return (
      <div className="animate-slide-in glass-panel" style={{ padding: '1.5rem', borderRadius: '16px', borderLeft: '4px solid #facc15', backgroundColor: '#ecfdf5', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <div>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#065f46', marginBottom: '0.25rem' }}>{t.merchant.variations.approveSuccessTitle.replace('{amount}', amount)}</h3>
          <p style={{ color: '#b8860b', fontSize: '0.9rem' }}>{t.merchant.variations.approveSuccessDesc.replace('{merchantName}', merchantName)}</p>
        </div>
      </div>
    );
  }
  ...
  if (status === "REJECTED") {
    return (
      <div className="animate-slide-in glass-panel" style={{ padding: '1.5rem', borderRadius: '16px', borderLeft: '4px solid #64748b', backgroundColor: '#f8fafc', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <div>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#334155', marginBottom: '0.25rem' }}>{t.merchant.variations.rejectSuccessTitle}</h3>
          <p style={{ color: '#475569', fontSize: '0.9rem' }}>{t.merchant.variations.rejectSuccessDesc}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-slide-in glass-panel" style={{ padding: '1.5rem', borderRadius: '16px', borderLeft: '4px solid #ef4444', backgroundColor: '#fef2f2', marginBottom: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
         <div>
            <div style={{ color: '#ef4444', fontWeight: 700, fontSize: '0.85rem', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              {t.merchant.variations.pendingAlert}
            </div>
            <h3 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#7f1d1d', marginBottom: '0.25rem' }}>{serviceName} - {t.merchant.variations.proposalTitle}</h3>
            <p style={{ color: '#991b1b', fontSize: '0.9rem' }}>{merchantName} {t.merchant.variations.initiatedChange}</p>
         </div>
         <span style={{ backgroundColor: '#fee2e2', color: '#ef4444', padding: '0.35rem 0.75rem', borderRadius: '2rem', fontSize: '0.8rem', fontWeight: 700 }}>
           {t.merchant.variations.pendingStatus}
         </span>
      </div>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
         ...
         <div style={{ flex: 1, minWidth: '200px' }}>
            <p style={{ color: '#7f1d1d', fontSize: '0.9rem', marginBottom: '0.5rem', lineHeight: 1.5 }}>
              <strong>{t.merchant.variations.proofLabel}</strong> {mockDesc}
            </p>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
               <span style={{ fontSize: '0.9rem', color: '#991b1b' }}>{t.merchant.variations.authorizedAmountLabel}</span>
      ...
      <div style={{ display: 'flex', gap: '0.75rem' }}>
         ...
         >
           {isProcessing ? t.merchant.variations.processingAuth : t.merchant.variations.approveBtn}
         </button>
         <button 
         ...
         >
           {t.merchant.variations.rejectBtn}
         </button>
      </div>
    </div>
  );
