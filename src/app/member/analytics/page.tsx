...
export default function AnalyticsPage() {
  ...
  if (!data) return <div>{t.merchant.merchant_reviews.empty}</div>;
  ...
  return (
    <div className="animate-fade-in">
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-primary)' }}>{t.merchant.merchant_analytics.title}</h1>
        <p style={{ color: 'var(--text-secondary)' }}>{t.merchant.merchant_analytics.subtitle}</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', marginBottom: '3rem' }}>
        ...
        <div className="glass-panel" style={{ padding: '2rem', borderRadius: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '2rem' }}>
             ...
             <h2 style={{ fontWeight: 800 }}>{t.merchant.merchant_analytics.ratingDistribution}</h2>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
             <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{t.merchant.merchant_analytics.totalReviews.replace('{count}', data.totalReviews.toString())} {data.totalReviews} {t.merchant.merchant_analytics.totalReviews}</p>
             </div>
        ...
        <div className="glass-panel" style={{ padding: '2rem', borderRadius: '24px', backgroundColor: 'rgba(99, 102, 241, 0.03)', border: '1px solid rgba(99, 102, 241, 0.2)' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
              ...
              <h2 style={{ fontWeight: 800 }}>{t.merchant.merchant_analytics.marketPosition.title}</h2>
           </div>
           
           <div style={{ marginBottom: '1.5rem' }}>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>{t.merchant.merchant_analytics.marketPosition.ranking}</p>
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)' }}>
                 {t.merchant.merchant_analytics.marketPosition.topPercent.replace('{percent}', (100 - mPos.percentile).toString()).replace('{category}', (mPos.mainCategory || "").toString())}
              </div>
           </div>

           <div style={{ padding: '1rem', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.03)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.85rem' }}>
                 <span style={{ color: 'var(--text-secondary)' }}>{t.merchant.merchant_analytics.marketPosition.categoryAvg}</span>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                 <span style={{ color: 'var(--text-secondary)' }}>{t.merchant.merchant_analytics.marketPosition.cityRank}</span>
        ...
        <div className="glass-panel" style={{ padding: '2rem', borderRadius: '24px', background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.05), rgba(99, 102, 241, 0.05))' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
              ...
              <h2 style={{ fontWeight: 800 }}>{t.merchant.merchant_analytics.sentiment.title}</h2>
           </div>
           <div style={{ display: 'flex', gap: '1rem' }}>
              <div style={{ flex: 1, padding: '1rem', borderRadius: '12px', backgroundColor: 'rgba(16, 185, 129, 0.1)', textAlign: 'center' }}>
                 ...
                 <p style={{ fontSize: '0.7rem', color: '#facc15' }}>{t.merchant.merchant_analytics.sentiment.positive}</p>
              </div>
              <div style={{ flex: 1, padding: '1rem', borderRadius: '12px', backgroundColor: 'rgba(239, 68, 68, 0.1)', textAlign: 'center' }}>
                 ...
                 <p style={{ fontSize: '0.7rem', color: '#ef4444' }}>{t.merchant.merchant_analytics.sentiment.negative}</p>
              </div>
           </div>
        </div>
      </div>
      ...
      <div className="glass-panel" style={{ padding: '1.5rem 2rem', borderRadius: '20px', marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '2rem' }}>
         <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', minWidth: '120px' }}>
            ...
            <span style={{ fontWeight: 800, fontSize: '0.85rem' }}>{t.merchant.merchant_analytics.traits}</span>
         </div>
         <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem' }}>
            ...
            )) : <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>{t.merchant.merchant_analytics.waitForData}</span>}
         </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 1fr) 2fr', gap: '2rem' }}>
         <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '20px' }}>
            <h3 style={{ fontWeight: 800, marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
               <TrendingUp size={18} /> {t.merchant.merchant_analytics.insights.title}
            </h3>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
               {t.merchant.merchant_analytics.insights.desc.replace('{category}', (mPos.mainCategory || "").toString()).replace('{trait}', (data.topKeywords[0] || "服務品質").toString())}
            </p>
         </div>
         ...
         <div className="glass-panel" style={{ padding: '2rem', borderRadius: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
               <h3 style={{ fontWeight: 800, display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <MessageCircle size={20} color="var(--accent-color)" /> {t.merchant.merchant_analytics.management.title}
               </h3>
               ...
               <div style={{ display: 'flex', gap: '0.5rem' }}>
                  {["ALL", "POSITIVE", "NEGATIVE", "UNREPLIED"].map(f => (
                    ...
                    >
                      {f === "ALL" ? t.merchant.merchant_analytics.management.filterAll : 
                       f === "POSITIVE" ? t.merchant.merchant_analytics.management.filterPositive : 
                       f === "NEGATIVE" ? t.merchant.merchant_analytics.management.filterNegative : 
                       t.merchant.merchant_analytics.management.filterUnreplied}
                    </button>
                  ))}
               </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
               {filteredReviews.length === 0 ? (
                 <p style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-secondary)' }}>{t.merchant.merchant_analytics.management.empty}</p>
               ) : (
                    ...
                    <div key={r.id} style={{ padding: '1.25rem', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '16px', position: 'relative' }}>
                       ...
                       {r.reply ? (
                         <div style={{ padding: '1rem', borderRadius: '12px', backgroundColor: 'rgba(15, 118, 110, 0.05)', borderLeft: '3px solid #0f766e' }}>
                            <p style={{ fontSize: '0.75rem', fontWeight: 800, color: '#0f766e', marginBottom: '0.25rem' }}>{t.merchant.merchant_analytics.management.yourReplyLabel}</p>
                         ...
                         <div style={{ position: 'relative' }}>
                            <div style={{ display: 'flex', gap: '0.75rem' }}>
                               <input 
                                 ...
                                 placeholder={t.merchant.merchant_analytics.management.placeholder} 
                               ...
                               >
                                  {replying === r.id ? "..." : t.merchant.merchant_analytics.management.replyBtn}
                               </button>
                            </div>
                            ...
                            {drafts[r.id] && (
                              <div className="glass-panel" style={{ position: 'absolute', bottom: '100%', left: 0, right: 0, zIndex: 100, padding: '1rem', marginBottom: '0.5rem', boxShadow: '0 10px 25px rgba(0,0,0,0.2)', border: '1px solid var(--accent-color)' }}>
                                 <p style={{ fontSize: '0.75rem', fontWeight: 800, marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                                    <Sparkles size={12} color="var(--accent-color)" /> {t.merchant.merchant_analytics.management.aiDrafts}
                                 </p>
