    ...
    <div className="auth-page-wrapper">
      <div className="auth-card">
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          <h1 className="hero-title" style={{ fontSize: '2.5rem', marginBottom: '0.75rem', display: 'block' }}>{t.auth.login.title}</h1>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column' }}>
          <div className={`input-group ${revealed ? 'revealed' : ''}`} style={{ animationDelay: '100ms' }}>
            ...
            <div className="input-wrapper">
              ...
              <input type="email" name="email" className="premium-input" placeholder={t.auth.login.emailPlaceholder} required disabled={loading} />
            </div>
          </div>
          
          <div className={`input-group ${revealed ? 'revealed' : ''}`} style={{ animationDelay: '150ms' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
              <label style={{ marginBottom: 0 }}>{t.auth.login.passwordLabel}</label>
              <Link href="/auth/forgot-password" style={{ fontSize: '0.85rem', color: 'var(--accent-color)', fontWeight: 600 }}>{t.auth.login.forgotPassword}</Link>
            </div>
            <div className="input-wrapper">
        ...
        <div className="divider">
          <span>{revealed ? t.auth.login.or : ''}</span>
        </div>
        ...
        <div style={{ textAlign: 'center', fontSize: '0.9rem', color: 'var(--text-muted)', marginTop: '2rem' }}>
          {t.auth.login.navToRegister} <Link href="/auth/register" style={{ color: 'var(--accent-color)', fontWeight: 800 }}>{t.auth.login.createAccount}</Link>
        </div>
      </div>
    </div>
  );
