...
export default function MerchantSchedulePage() {
  ...
  // Custom Slot State
  const [customAction, setCustomAction] = useState<"ADD" | "BLOCK">("ADD");
  ...
  const handleAddCustom = async () => {
    ...
    const res = await addCustomScheduleSlot(dStr, customStart, customEnd, isAvail);
    if (res.success) {
      setMsg({ type: 'success', text: "Schedule Slot customized successfully." });
      await loadCustomSlotsForDay(selectedDay);
      setTimeout(() => setMsg(null), 2000);
    } else {
      setMsg({ type: 'error', text: "Failed to customize slot." });
    }
    setSaving(false);
  };
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', direction: isRTL ? 'rtl' : 'ltr' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selectedDay ? (isRTL ? '400px 1fr' : '1fr 400px') : '1fr', gap: '2rem', transition: 'all 0.3s' }}>
        ...
        {selectedDay && (
          <div className="glass-panel animate-fade-left" style={{ padding: '2rem', borderRadius: '32px', backgroundColor: 'var(--surface-1)', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div style={{ padding: '1.25rem', backgroundColor: 'var(--surface-2)', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
                <h4 style={{ fontSize: '0.9rem', fontWeight: 800, marginBottom: '1rem', color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Add Manual Schedule Block</h4>
                <div style={{ display: 'grid', gap: '1rem' }}>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button type="button" onClick={() => setCustomAction("ADD")} style={{flex: 1, padding: '0.5rem', borderRadius: '8px', border: '1px solid ' + (customAction==="ADD" ? '#10b981' : 'var(--border-color)'), backgroundColor: customAction==="ADD" ? '#10b98122' : 'transparent', color: customAction==="ADD" ? '#10b981' : 'var(--text-muted)', fontWeight: 800, fontSize: '0.8rem'}}>Add Free Slot</button>
                    <button type="button" onClick={() => setCustomAction("BLOCK")} style={{flex: 1, padding: '0.5rem', borderRadius: '8px', border: '1px solid ' + (customAction==="BLOCK" ? '#ef4444' : 'var(--border-color)'), backgroundColor: customAction==="BLOCK" ? '#ef444422' : 'transparent', color: customAction==="BLOCK" ? '#ef4444' : 'var(--text-muted)', fontWeight: 800, fontSize: '0.8rem'}}>Block Slot</button>
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    <input type="time" value={customStart} onChange={e => setCustomStart(e.target.value)} className="input-field" style={{ flex: 1, padding: '0.5rem', borderRadius: '8px' }} />
                    <span style={{ color: 'var(--text-muted)' }}>-</span>
                    <input type="time" value={customEnd} onChange={e => setCustomEnd(e.target.value)} className="input-field" style={{ flex: 1, padding: '0.5rem', borderRadius: '8px' }} />
                  </div>
                  <button onClick={handleAddCustom} disabled={saving} className="btn btn-primary" style={{ padding: '0.6rem', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                    {saving ? <Loader2 className="animate-spin" size={16} /> : <Plus size={16} />} Save Schedule
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                {customSlots.length === 0 ? (
                  ...
                  <div style={{ display: 'grid', gap: '0.5rem' }}>
                    {customSlots.map(c => (
                      <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', borderRadius: '12px', border: `1px solid ${c.isAvailable ? '#10b98144' : '#ef444444'}`, backgroundColor: c.isAvailable ? '#10b98111' : '#ef444411' }}>
                        <div>
                          <div style={{ fontSize: '0.85rem', fontWeight: 800, color: c.isAvailable ? '#10b981' : '#ef4444' }}>
                            {c.isAvailable ? "Availability Override" : "Blocked Time"}
                          </div>
      ...
      <div style={{ marginTop: '2.5rem', display: 'flex', gap: '2rem', alignItems: 'center', justifyContent: 'center', flexDirection: isRTL ? 'row-reverse' : 'row' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-secondary)' }}>
          <div style={{ width: '12px', height: '12px', borderRadius: '3px', backgroundColor: 'rgba(99, 102, 241, 0.08)', border: '2px solid #6366f1' }}></div>
          Selected Day
        </div>
      </div>
