        ...
        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
             ...
             <input title={t?.merchant?.avatar?.hint || "Update Avatar"} type="file" accept="image/*" onChange={(e) => {
                ...
                if (file) {
                  ...
                  reader.onloadend = async () => {
                     ...
                     alert(t?.merchant?.avatar?.success || "Updated Successfully");
                  };
          ...
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)' }}>
              {t?.merchant?.welcome?.replace('{name}', stats?.companyName || t?.common?.user || "Member") || "Welcome back"}
            </p>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem' }}>
           <Link href="/services/results" className="btn" style={{ padding: '0.6rem 1rem', fontSize: '0.9rem', backgroundColor: 'var(--surface-1)', border: '1px solid var(--border-color)', color: 'var(--accent-color)', borderRadius: '12px', textDecoration: 'none' }}>{t?.merchant?.previewProfile || "Preview Profile"}</Link>
           <Link href="/dashboard/merchant/services" className="btn" style={{ padding: '0.6rem 1.2rem', fontSize: '0.9rem', textDecoration: 'none', backgroundColor: 'var(--accent-color)', color: 'var(--text-contrast)', borderRadius: '12px', fontWeight: 700 }}>{t?.merchant?.manageServices || "Manage Services"}</Link>
        </div>
      </header>

      {/* Stats Grid - Obsidian Cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem', marginBottom: '3rem' }}>
        <StatCard title={t?.merchant?.stats?.totalBookings || "Total Bookings"} value={stats?.totalBookings} icon={<Briefcase size={24} color="#d4af37" />} color="#d4af37" label={t?.merchant?.stats?.totalJobs || "Total Jobs"} />
        <StatCard title={t?.merchant?.stats?.rating || "Rating"} value={stats?.rating?.toFixed(1) || "5.0"} icon={<Star size={24} color="#d4af37" fill="#d4af37" />} color="#d4af37" label={`${stats?.reviews || 0} ${t?.merchant?.stats?.reviews || "Reviews"}`} />
        <StatCard title={t?.merchant?.stats?.pendingBalance || "Pending Balance"} value={`£${(stats?.pendingBalance || 0).toFixed(2)}`} icon={<Clock size={24} color="#d4af37" />} color="#d4af37" label={t?.merchant?.stats?.escrowHeld || "In Escrow"} />
        <StatCard title={t?.merchant?.stats?.availableBalance || "Available Balance"} value={`£${(stats?.availableBalance || 0).toFixed(2)}`} icon={<Wallet size={24} color="#d4af37" />} color="#d4af37" label={t?.merchant?.stats?.availableNow || "Available Now"} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem' }}>
        ...
        <section style={{ padding: '1.5rem', borderRadius: '24px', backgroundColor: 'var(--surface-1)', border: '1px solid var(--border-color)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            ...
            <Link href="#" style={{ fontSize: '0.85rem', color: 'var(--accent-color)', fontWeight: 600 }}>{t?.merchant?.bookings?.viewAll || "View All"}</Link>
          </div>

          {bookings.length === 0 ? (
             <div style={{ textAlign: 'center', padding: '4rem 0', opacity: 0.3 }}>
               <AlertCircle size={48} style={{ margin: '0 auto 1rem' }} />
               <p>{t.merchant.bookings.empty}</p>
             </div>
          ) : (
                  ...
                  <div style={{ textAlign: 'right' }}>
                    {booking.status === 'COMPLETED' ? (
                      <span style={{ color: '#d4af37', display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.85rem', fontWeight: 700, justifyContent: 'flex-end' }}>
                         <CheckCircle2 size={16} /> {t.merchant.bookings.completed}
                      </span>
                    ) : (
                      <div style={{ display: 'flex', gap: '0.5rem', flexDirection: 'column', alignItems: 'flex-end' }}>
                        {booking.status === 'CONFIRMED' && (
                          ...
                          >
                             <PlusCircle size={14} style={{ marginRight: '0.3rem' }} /> {t.merchant.bookings.actions.variation}
                          </button>
                        )}
                        ...
                        >
                          {updatingId === booking.id 
                            ...
                            : booking.status === 'PENDING' 
                              ? t.merchant.bookings.actions.confirm 
                              : booking.isEducation 
                                ? t.merchant.bookings.actions.complete 
                                : t.merchant.bookings.actions.completeAndPay
                          }
                        </button>
                      </div>
                    )}
                        ...
                        {booking.variations.map((v: any) => (
                          <div key={v.id}>
                            <div style={{ fontWeight: 700, display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
                              <span>{t.merchant.variations.label}: {v.description}</span>
                              <span style={{ color: '#d4af37' }}>£{v.amount}</span>
                            </div>
                            
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem' }}>
                              <span style={{ fontSize: '0.8rem', color: '#666' }}>
                                {t.merchant.variations.status}: {v.status === 'PENDING' ? t.merchant.variations.pending : v.status === 'APPROVED' ? t.merchant.variations.approved : t.merchant.variations.rejected}
                              </span>
                              {v.status === 'DISPUTED' && (!booking.dispute || booking.dispute.status !== 'RESOLVED') && (
                                <span style={{ color: '#ef4444', fontWeight: 800, fontSize: '0.75rem' }}>{t.merchant.arbiterActive}</span>
                              )}
                            </div>
                            ...
                            {booking.dispute && booking.dispute.status === 'RESOLVED' && (
                              <div style={{ marginTop: '0.5rem', padding: '0.75rem', borderRadius: '8px', backgroundColor: 'rgba(212, 175, 55, 0.05)', border: '1px solid rgba(212, 175, 55, 0.2)' }}>
                                <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#d4af37', marginBottom: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                                  <Cpu size={14} /> {t.merchant.aiReason}
                                </div>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
           <div style={{ padding: '1.5rem', borderRadius: '24px', backgroundColor: '#1a1a1a', border: '1px solid #d4af37' }}>
              <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
                ...
                <h3 style={{ fontSize: '1rem', fontWeight: 800, color: '#d4af37' }}>{t.merchant.tipsTitle}</h3>
              </div>
              <p style={{ fontSize: '0.85rem', color: '#ccc', lineHeight: 1.5 }}>
                {t.merchant.tips.growth}. {t.merchant.tipsExtra}
              </p>
           </div>

           <div style={{ padding: '1.5rem', borderRadius: '24px', backgroundColor: '#0c0c0c', border: '1px solid #222' }}>
              <h3 style={{ fontSize: '1rem', fontWeight: 800, marginBottom: '1.25rem', color: '#fff' }}>{t.merchant.quickLinks.title}</h3>
              <div style={{ display: 'grid', gap: '0.75rem' }}>
                <QuickLink label={t.merchant.quickLinks.schedule} />
                <QuickLink label={t.merchant.quickLinks.earnings} />
                <QuickLink label={t.merchant.quickLinks.support} />
              </div>
           </div>
        </aside>
      </div>
       ...
       <div style={{ width: '100%', maxWidth: '480px', padding: '2.5rem', borderRadius: '32px', backgroundColor: 'var(--surface-1)', border: '1px solid var(--glass-border)', color: 'var(--text-primary)', boxShadow: 'var(--shadow-xl)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <h3 style={{ fontSize: '1.5rem', fontWeight: 900, color: '#fff', letterSpacing: '-0.02em' }}>{t?.merchant?.modal?.title || "Initialize Calibration"}</h3>
            </div>
          ...
          <form onSubmit={onSubmit}>
            <div style={{ display: 'grid', gap: '1.5rem' }}>
              <div style={{ backgroundColor: '#050505', padding: '1.25rem', borderRadius: '20px', border: '1px solid #1a1a1a' }}>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 800, marginBottom: '0.75rem', color: '#d4af37', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t?.merchant?.modal?.amount || "Amount"}</label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 800, marginBottom: '0.5rem', color: '#999', paddingLeft: '0.5rem' }}>{t?.merchant?.modal?.reason || "Reasoning"}</label>
                <textarea required value={desc} onChange={e => setDesc(e.target.value)} placeholder={t?.merchant?.modal?.reasonPlaceholder || "Explain change..."} style={{ width: '100%', padding: '1rem', borderRadius: '16px', border: '1px solid #222', backgroundColor: '#050505', color: '#fff', height: '100px', resize: 'none', fontSize: '0.95rem', lineHeight: 1.5, outline: 'none' }} />
              </div>

              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 800, marginBottom: '0.5rem', color: '#999', paddingLeft: '0.5rem' }}>
                  {t?.merchant?.modal?.photo || "Evidence"} <span style={{ color: '#d4af37', fontSize: '0.7rem' }}>(ELITE REQUIREMENT)</span>
                </label>
              ...
              <button disabled={loading} type="submit" style={{ 
                ...
                {loading ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                    ...
                    {t?.merchant?.submitting || "Transmitting..."}
                  </div>
                ) : (t?.merchant?.modal?.submit || "Push Change")}
              </button>
            </div>
          </form>
       </div>
