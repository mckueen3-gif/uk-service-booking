  ...
  return (
    <div className="animate-fade-up">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '2.2rem', fontWeight: 900, color: 'var(--text-primary)' }}>{t.merchant.merchant_reviews.title}</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>{t.merchant.merchant_reviews.subtitle}</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem', marginBottom: '3rem' }}>
        <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '20px', border: '1.5px solid var(--border-color)', backgroundColor: 'var(--surface-1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--text-secondary)' }}>{t.merchant.merchant_reviews.total}</span>
        ...
        <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '20px', border: '1.5px solid var(--border-color)', backgroundColor: 'var(--surface-1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--text-secondary)' }}>{t.merchant.merchant_reviews.positive}</span>
        ...
        <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '20px', border: '1.5px solid var(--border-color)', backgroundColor: 'var(--surface-1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <span style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--text-secondary)' }}>{t.merchant.merchant_reviews.negative}</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1rem', maxWidth: '1000px' }}>
        {merchant.reviews.length === 0 ? (
          <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
            {t.merchant.merchant_reviews.empty}
          </div>
        ) : (
