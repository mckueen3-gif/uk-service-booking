...
export async function sendPlatformEmail({
  ...
  try {
    const data = await resend.emails.send({
      from: 'ConciergeAI <noreply@conciergeai.uk>',
      to,
      subject,
      html,
    });
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
      ...
      <div style="padding: 30px;">
        ...
        <a href="${process.env.NEXTAUTH_URL}/dashboard/disputes/${disputeId}" style="display: inline-block; background-color: #0f172a; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; margin-top: 20px;">
          登入平台查看完整 JSON 分析報告
        </a>
      </div>
